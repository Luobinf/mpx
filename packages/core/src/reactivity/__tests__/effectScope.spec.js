...
import { reactive, effect, EffectScope, onScopeDispose, computed, ref, getCurrentScope } from '../src';
describe('reactivity/effect/scope', () => {
    ...
    it.skip('can not run an inactive scope', () => {
        let dummy, doubled;
        const counter = reactive({ num: 0 });
        const scope = new EffectScope();
        scope.run(() => {
    ...
    it.skip('should warn onScopeDispose() is called when there is no active effect scope', () => {
        const spy = jest.fn();
        const scope = new EffectScope();
        scope.run(() => {
            onScopeDispose(spy);
        ...
        onScopeDispose(spy);
        expect('[Vue warn] onScopeDispose() is called when there is no active effect scope to be associated with.').toHaveBeenWarned();
        scope.stop();
    ...
    it.skip('test with higher level APIs', () => __awaiter(void 0, void 0, void 0, function* () {
        const r = ref(1);
